...
const PATH_REGIONS=[
 {name:"ANCIENT FOREST",decor:["🌿","🌳","🦴"]},
 {name:"FERN PLAINS",decor:["🌱","🪨","🌾"]},
 {name:"ROCKY HIGHLANDS",decor:["⛰️","🪨","🦴"]},
 {name:"PREHISTORIC WETLANDS",decor:["🌊","🌿","🪵"]},
 {name:"LUSH VALLEY",decor:["🌴","🌿","🌳"]},
 {name:"PREDATOR TERRITORY",decor:["🦴","🪨","🌋"]}
];
...
function buildLongPath(){
  ...
  PATH_REGIONS.forEach((region,i)=>{
    ...
    el.innerHTML=`
      <div class="region-title"><span class="region-number">REGION ${i+1}</span>${region.name}</div>
      <div class="region-decor d1">${region.decor[0]}</div>
      <div class="region-decor d2">${region.decor[1]}</div>
      <div class="region-decor d3">${region.decor[2]}</div>`;
