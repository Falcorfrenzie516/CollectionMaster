...
function go(v){
  ...
  if(v==="path"){
    setTimeout(()=>scrollPathToCurrent(true),70);
  }else if(active){
...
function buildPath(theme){
  ...
  pathEngine=new PathEngine({
    ...
    svgEl:$("longPathSvg"),
    tokenEl:$("pathToken"),
    onNodeClick:(node,reward)=>showPathNode(node,reward)
  });
  pathEngine.build();
  render();
}

function showPathNode(node,reward){
  const text={pack:"Pack reward",cash:"Cash reward",special:"Special reward",expedition:"Expedition unlock",milestone:"Major milestone"}[reward]||"Travel node";
  modal(`<p style="color:#9bcf68;font-size:.7rem;letter-spacing:.16em;font-weight:900">${activePathTheme.title}</p><h2>Node ${node}</h2><p>${text}</p><p style="color:#a9b0b3">The reusable Path engine will attach this node to the active collection/event reward table.</p>`);
}
...
window.addEventListener("resize",()=>{if(pathEngine){buildPath(activePathTheme);setTimeout(()=>scrollPathToCurrent(false),40)}});
